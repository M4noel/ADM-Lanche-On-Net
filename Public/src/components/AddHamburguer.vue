<!-- <template>
  <div class="container">
    ...
    <form @submit.prevent="addBebida">
      ...
      <button type="submit">Adicionar Hamburger</button>

    
      <p v-if="showErrorMessage" class="error-message">Preencha todos os campos antes de enviar.</p>

      <p v-if="showSuccessMessage" class="success-message">Hamburger adicionada com sucesso!</p>
    </form>
  </div>
</template>
...
<template>
  <div class="container">
    ...
    <form @submit.prevent="addCombo" class="form">
      <div class="form-group">
        <input type="number" id="quantity" v-model="combosData.quantity" required>
      </div>
      <div class="form-group">
        <label for="name">Nome</label>
        <input type="text" id="name" v-model="combosData.name" required>
      </div>
      <div class="form-group">
        <label for="description">Descrição</label>
        <input type="text" id="description" v-model="combosData.description" required>
      </div>
      <div class="form-group">
        <label for="price">Preço</label>
        <input type="number" id="price" v-model="combosData.price" required>
      </div>
      <div class="form-group">
        <label for="image">Imagem</label>
        <input type="file" id="image" @change="handleFileUpload" required>
      ...
      <button type="submit" class="submit-button">Adicionar Hambúrguer</button>
    </form>
  </div>
</template>
...
export default {
  ...
  methods: {
    handleFileUpload(event) {
      this.combosData.image = event.target.files[0];
    },
    addCombo() {
      this.showErrorMessage = false;
      this.showSuccessMessage = false;
      ...
      const formData = new FormData();
      formData.append('quantity', this.combosData.quantity);
      formData.append('name', this.combosData.name);
      formData.append('description', this.combosData.description);
      formData.append('price', this.combosData.price);
      formData.append('file', this.combosData.image);
      ...
      axios.post('http://localhost:4000/combos', formData)
        ...
        .catch(error => {
          console.error('Erro ao adicionar a Hanburgue:', error);
          this.showErrorMessage = true;
        });
    },
  },
